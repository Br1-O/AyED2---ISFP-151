...
from src.lib.app.shared.utils.logs import log_info, log_error, log_warn

def run(service: ICustomerService):
    root = tk.Tk()
    root.title("Gestión de Clientes - NicBread")
    ...
    menubar.add_cascade(label="Navegar", menu=tab_menu)

    tab_menu.add_command(label="Listar Todos", command=lambda: notebook.select(0))
    tab_menu.add_command(label="Crear", command=lambda: notebook.select(1))
    tab_menu.add_command(label="Buscar", command=lambda: notebook.select(2))
    tab_menu.add_command(label="Modificar", command=lambda: notebook.select(3))
    tab_menu.add_command(label="Eliminar", command=lambda: notebook.select(4))
    ...
    def search_customer():
        try:
            ...
            if customer:
                ...
                # También rellenar el formulario de modificación
                entry_code_update.delete(0, tk.END)
                entry_code_update.insert(0, str(customer.code.value))

                entry_name_update.delete(0, tk.END)
                entry_name_update.insert(0, customer.name.value)

                entry_lastname_update.delete(0, tk.END)
                entry_lastname_update.insert(0, customer.lastname.value)

                entry_address_update.delete(0, tk.END)
                entry_address_update.insert(0, customer.address.value)
